...
export class CategorierView extends View {
    ...
    async doChangeCat(clickData){
      ...
      _.chooseActveItem(item)
      MainEventBus.trigger(_.componentName,'changeCat',category);
    }
    ...
    chooseActveItem(item){
	    item.parentNode.parentNode.parentNode.querySelectorAll('li').forEach((el)=>{el.classList.remove('active')})
    ...
    async editCat(clickData){
        ...
        for(let elem of categorierForm.elements){
            let prop = elem.name;
            if(prop == 'parent'){
                await _.setParentField(elem,category[`${prop}`])
                continue;
            }
            if(prop == 'image'){
                _.setImageField(elem,category[`${prop}`]);
                continue;
            }
            elem.value = category[`${prop}`] ? category[`${prop}`] : '';
        }
        if(category['props']){
            await _.setPropsTable(category['props'])
        }
