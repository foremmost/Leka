...
class Customers {
    ...
    function getTempUserByMail($userData){
        $mail = $userData->mail;
        $sql = "
                SELECT `mail`,`id`
                FROM `temp_users`
                ...
                ORDER BY `mail` DESC
		";
        $userInfo = $this->db->prepare($sql,'s',[$mail])->fetch_assoc();
        if(!empty($userInfo)){
            return $userInfo;
        }
        return null;
	...
	function addMeta($uId,$userData){
		...
		$login = $this->db->filter($userData['login']);
		$sql = "
			INSERT INTO users_meta (`u_id`,`name`,`second_name`,`description`,`email`)
			VALUES(?,?,?,?,?)
		";
		$this->db->prepare($sql,'issss',[
				$uId,
				$name,
				$second_name,
				$description,
    ...
    function registerUser($userData){
	    $userData->add_date = date('Y-m-d H:i:s');
        $userData->id = $this->db->insert([
        ...
        return [
            'status'=>'fail',
            'failText'=>'temp user creating error',
